...
public class OmdbClientTests {
    ...
    @BeforeEach
    public void startWireMock() throws MalformedURLException {
        wireMockServer = new WireMockServer(wireMockConfig().dynamicHttpsPort().dynamicPort()); //No-args constructor will start on port 8080, no HTTPS
        wireMockServer.start();
        log.info("Started wiremock on " + wireMockServer.port());
        configureFor(this.wireMockServer.port());
        url = new URL("http://localhost:" + wireMockServer.port());
    }
    ...
    public void testSearch() {
        ...
        OmdbSearchResult search = client.search(searchTerm).blockingGet();
        log.info("Found: "+search);
        assertEquals(115, search.totalResults);
